...
function Navbar() {
  ...
  return (
   <>
   <div>
    <div className={`navbar bg-base-400 z-5  dark:bg-slate-900 dark:text-white h-10 ${sticky?"sticky-navbar shadow-md bg-base-200  dark:bg-slate-600 dark:text-white duration-200 transition-all ease-in-out":""}`}>
        <div className="navbar-start">
          <div className="dropdown">
            <div tabIndex={0} role="button" className="btn btn-ghost lg:hidden">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /></svg>
            </div>
            <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2  text-zinc-700 w-30 text-lg font-bold shadow bg-base-100 rounded-box w-52  flex flex-col justify-center items-center">
            {navItems}
            </ul>
          </div>
          <a className="btn dark:bg-slate-900 dark:text-white btn-ghost md:mx-24  mx-12 text-xl text-zinc-700 font-bold cursor-pointer">BookBound</a>
        </div>
        <div className="navbar-center hidden lg:flex my-2">
          <ul className="menu menu-horizontal px-1 text-gray-700  dark:bg-slate-900 dark:text-white  text-lg font-semibold flex justify-evenly1 space-x-1">
            {navItems}
          </ul>
        </div>
